...
PAYOUTBV ;
 ...
 ;If more than one of these present, loop thourgh the  
 K ^TMP("PXRHM",$J)
 K ^TMP("PXRM",$J)
 N N,HEPA,HEPB,VARICEL,MUMPS,MEASLES,RUBELLA S (N,HEPA,HEPB,VARICEL,MUMPS,MEASLES,RUBELLA)=0
 K ^TMP("PXRHM",$J)
 D MAIN^PXRM(DFN,264,0)
 I $G(^TMP("PXRHM",$J,264,"VIMM-HEPATITIS B DIAGNOSIS"))["DUE NOW" S N=N+1 S HEPB=1
 ;W "HEPB=",HEPB,!
 K ^TMP("PXRHM",$J)
 D MAIN^PXRM(DFN,265,0)
 I $G(^TMP("PXRHM",$J,265,"VIMM-HEPATITIS A DIAGNOSIS"))["DUE NOW" S N=N+1 S HEPA=1
 ;W "HEPA=",HEPA,!
 K ^TMP("PXRHM",$J)
 D MAIN^PXRM(DFN,266,0)
 I $G(^TMP("PXRHM",$J,266,"VIMM-VARICELLA DIAGNOSIS"))["DUE NOW" S N=N+1 S VARICEL=1
 ;W "VARICEL=",VARICEL,!
 K ^TMP("PXRHM",$J)
 D MAIN^PXRM(DFN,267,0)
 I $G(^TMP("PXRHM",$J,267,"VIMM-MUMPS DIAGNOSIS"))["DUE NOW" S N=N+1 S MUMPS=1
 ;W "MUMPS=",MUMPS,!
 K ^TMP("PXRHM",$J)
 D MAIN^PXRM(DFN,268,0)
 I $G(^TMP("PXRHM",$J,268,"VIMM-MEASLES DIAGNOSIS"))["DUE NOW" S N=N+1 S MEASLES=1
 ;W "MEASLES=",MEASLES,!
 K ^TMP("PXRHM",$J)
 D MAIN^PXRM(DFN,269,0)
 I $G(^TMP("PXRHM",$J,269,"VIMM-RUBELLA DIAGNOSIS"))["DUE NOW" S N=N+1 S RUBELLA=1
 ;W "RUBELLA=",RUBELLA,!
 K ^TMP("PXRHM",$J)
 I N=0 Q
 E  D
...
HEPB ;
 D MAIN^PXRM(DFN,264,0)
 I $G(^TMP("PXRHM",$J,264,"VIMM-HEPATITIS B DIAGNOSIS"))["DUE NOW" D
 .S C0IPOA("UUIDA")=$$UUID^C0IUTIL
 .S C0IPOA("timeProblem")=$$FMDTOUTC^C0IUTIL(DT)
...
HEPA ;
 D MAIN^PXRM(DFN,265,0)
 I $G(^TMP("PXRHM",$J,265,"VIMM-HEPATITIS A DIAGNOSIS"))["DUE NOW" D
 .S C0IPOA("UUIDA")=$$UUID^C0IUTIL
 .S C0IPOA("timeProblem")=$$FMDTOUTC^C0IUTIL(DT)
...
VARICEL ;
 D MAIN^PXRM(DFN,266,0)
 I $G(^TMP("PXRHM",$J,266,"VIMM-VARICELLA DIAGNOSIS"))["DUE NOW" D
 .S C0IPOA("UUIDA")=$$UUID^C0IUTIL
 .S C0IPOA("timeProblem")=$$FMDTOUTC^C0IUTIL(DT)
...
MUMPS ;
 D MAIN^PXRM(DFN,267,0)
 I $G(^TMP("PXRHM",$J,267,"VIMM-MUMPS DIAGNOSIS"))["DUE NOW" D
 .S C0IPOA("UUIDA")=$$UUID^C0IUTIL
 .S C0IPOA("timeProblem")=$$FMDTOUTC^C0IUTIL(DT)
 .S C0IPOA("codeICD9")="052.9"
...
MEASLES ;
 D MAIN^PXRM(DFN,268,0)
 I $G(^TMP("PXRHM",$J,268,"VIMM-MEASLES DIAGNOSIS"))["DUE NOW" D
 .S C0IPOA("UUIDA")=$$UUID^C0IUTIL
 .S C0IPOA("timeProblem")=$$FMDTOUTC^C0IUTIL(DT)
...
RUBELLA ;
 D MAIN^PXRM(DFN,269,0)
 I $G(^TMP("PXRHM",$J,269,"VIMM-RUBELLA DIAGNOSIS"))["DUE NOW" D
 .S C0IPOA("UUIDA")=$$UUID^C0IUTIL
 .S C0IPOA("timeProblem")=$$FMDTOUTC^C0IUTIL(DT)
